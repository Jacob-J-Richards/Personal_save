...
The trend is positive with a correlation coefficient of 0.9840177 which
is a strong correlation but almost perfect. The relationship between
these variables has no disturbing patters of increasing variability as
the predictor variable increases as the last one did.
...
The trend is positive with a correlation coefficient of 0.7153432 which
is a moderate correlation. The relationship between these variables has
a very obvious curvature.
...
Opening is the best choice as predictor variable. The relationship
between Opening and Us Revenue has a positive correlation coefficient
that is almost perfect (.98) and the scatter plot indicates no pattern
other than positive linearity. Budget is completely unsuitable as the
residuals of the fitted values would increase as budget increased.
Theaters is unsuitable as it's relationship is non-linear.
...
The y-intercept in this model is not meaningful as it's p-value for
testing the null hypothesis that the intercept is equal to zero is
greater than 0.05 and thus fails to reject the null hypothesis that the
true y-intercept is zero.
...
The R-squared produced by the model summary is .9683 which means that
about 97% of the variance of the response variable US Revenue is
explained by the value of the Predictor variable opening.
...
The linear model predictes that the movie Get Smart which had an opening
