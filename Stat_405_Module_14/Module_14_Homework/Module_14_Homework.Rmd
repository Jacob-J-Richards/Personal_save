...
a.  Player 2 insists that he is the better player and suggests to
    ...
    They are both playing the same level, this is paired 2 tailed
    t-test.
...
transposed_df$Player1 <- as.numeric(transposed_df$Player1)
transposed_df$Player2 <- as.numeric(transposed_df$Player2)
write.csv(transposed_df,file="pairedtest.csv")

t.test(transposed_df$Player1, transposed_df$Player2, paired=TRUE)
```
...
```{r}
wilcox.test(transposed_df$Player1, transposed_df$Player2, paired=TRUE)
```

The resulting p-value is greater than 0.05, we therefore fail to reject
the null hypothesis the true location shift is equal to zero.
